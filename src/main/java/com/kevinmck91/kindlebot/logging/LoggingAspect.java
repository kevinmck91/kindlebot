...
public class LoggingAspect {
	...
	@Pointcut("execution(public * com.kevinmck91.kindlebot..*(..))")
	...
	public Object logExecutionTimeAndDetails(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			// If the method is fetching API data (like GithubItem[])
			if (result instanceof GithubItem[]) {
				GithubItem[] items = (GithubItem[]) result;
				logger.info("📦 API call returned {} items", items.length);
			} else if (result instanceof ResponseEntity) {
				ResponseEntity<?> response = (ResponseEntity<?>) result;
				logger.info("📡 HTTP Response: status={}, body={}", response.getStatusCode(), response.getBody());
			}
